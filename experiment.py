...
class CooperationGame(Experiment):

    def __init__(self, session):
        super(CooperationGame, self).__init__(session)

        self.task = "The Cooperation Game"
        self.verbose = True
        self.experiment_repeats = 21
        self.contactAWS = False
        self.practice_repeats = 3
        self.agent = CooperationAgent
        self.levels = [1, 2, 3]*self.experiment_repeats
        self.practice_levels = [1, 2, 3]
        self.min_acceptable_performance = 0
        self.generation_size = 20
        self.network = lambda: CooperationGenerational(generations=40,
                                                       generation_size=self.generation_size,
                                                       initial_source=False)
        self.group_size = 10
        self.bonus_payment = 1.0
        self.cost_of_cooperation = 10
        self.pot_multiplier = 2
        self.cost_of_punishment = 1
        self.value_of_punishment = 5
    ...
    def setup(self):
        super(CooperationGame, self).setup()
        for net in self.networks():
            net.group_size = self.group_size
            if net.role == "practice":
                net.levels = self.practice_levels[net.id - 1]
            elif net.role == "experiment":
                net.levels = self.levels[net.id - self.practice_repeats - 1]
            SummarySource(network=net)
    ...
    def data_check(self, participant):
        ...
        if (len(nets) != len(set(nets))
                or len(nets) != self.experiment_repeats + self.practice_repeats):
            return False
        ...
        for node in nodes:
            ...
            levels = node.network.levels
            decisions = node.infos(type=Decision)

            if levels != len(decisions):
                return False

            for d in decisions:
                if int(d.contents) not in [1, 0]:
                    return False
...
class CooperationGenerational(DiscreteGenerational):

    __mapper_args__ = {"polymorphic_identity": "cooperation_generational"}
    ...
    def group_size(self, group_size):
        self.property5 = repr(group_size)
    ...
    def add_node(self, node):
        ...
        if current_generation > 0:
            source = SummarySource.query.filter_by(network_id=self.id).one()
            source.connect(whom=node)
            source.send_summary(to_whom=node)
            node.receive()
...
def get_num_trials():
    exp = CooperationGame(db.session)
    data = {"status": "success", "experiment_repeats": exp.experiment_repeats, "practice_repeats": exp.practice_repeats}
    return Response(dumps(data), status=200, mimetype='application/json')


@extra_routes.route("/levels/<int:network_id>", methods=["GET"])
def get_levels(network_id):
    net = Network.query.filter_by(id=network_id).one()
    data = {"status": "success", "levels": net.levels}
    return Response(dumps(data), status=200, mimetype='application/json')
